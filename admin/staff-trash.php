...
foreach($checkUserProjects as $checkUserProject){
	...
	$project = projects::findByProjectId($userproject_id);
...
if($userproject_id){
		  $updateProjectUser="UPDATE projects SET s_ids = '$output' WHERE p_id = '$userproject_id'";
	  $updateDone=mysqli_query($connect, $updateProjectUser);
if($updateDone){
	$success = 0;
} else{
	$success = 1;
...
/* Remove User From Projects End*/
	  if($userDeleted){
		 if($success == 0){
		 header("Location:staff-trash.php?message=success");
		 } else{
			  header("Location:staff-trash.php?message=fail");
...
$flag=0;
		if($flag==0)
		{
			...
			$saveUser=$user->save();
			if($saveUser){
				header("Location:staff-trash.php?message=successrestore");
			}else {
header("Location:staff-trash.php?message=restorefail");
			}
		}
...
<div class="row project-dash">
				...
				<div class="pm-heading"><h2><?php echo $lang['Trash Staff']; ?></h2><span><?php echo $lang['All Deleted staff']; ?></span></div>
...
<?php $recentlyRegisteredUsers=user::findBySql("SELECT * FROM users WHERE accountStatus = 3 AND status = 1");?>
				</div> 
...
<ul>
				<li class="pm-trashbox"><a href="staff.php">
				<?php echo $lang['All Staff']; ?>
				</a></li>
				</ul>
				</div>
				</div>
				<ul class="deskvisible">
<li class="cproject"><a href="add-staff.php"><?php echo $lang['Add Staff']; ?><span>+</span></a></li>
				<li class="pm-trashbox"><a href="staff.php"><?php echo $lang['All Staff']; ?></a></li>
				</ul>
			</div>
			</div>
